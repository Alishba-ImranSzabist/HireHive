...
class _FreelancerProfileScreenState extends State<FreelancerProfileScreen> {
  ...
  void _openEditDialog() {
    ...
    final skillsCtrl = TextEditingController(text: auth.skills ?? '');
        ...
        builder: (ctx, setDlg) => AlertDialog(
          ...
          content: SingleChildScrollView(
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              ...
              SizedBox(height: 12),
              _field(skillsCtrl, "Skills",    Icons.code_outlined),
              SizedBox(height: 12),
              TextField(
            ...
            ElevatedButton(
              ...
              onPressed: saving ? null : () async {
                ...
                final result = await UserService(auth).updateProfile(
                  ...
                  skills: skillsCtrl.text.trim(), company: auth.company ?? '', bio: bioCtrl.text.trim(),
                );
                setDlg(() => saving = false);
                Navigator.pop(ctx);
                setState(() {});
            ...
            child: Column(
              children: [
                  ...
                  children: [
                    Container(
                      ...
                      child: CircleAvatar(
                        ...
                        child: !hasImage ? Icon(Icons.person_outline, size: 50, color: Colors.white) : null,
                      ),
                    ),
                    GestureDetector(
                ...
                Container(
                  ...
                  child: Text("Freelancer", style: TextStyle(color: Colors.white, fontSize: 13, fontWeight: FontWeight.w600)),
                ),
                // Skills chips preview
                if ((auth.skills ?? '').isNotEmpty) ...[
                  SizedBox(height: 12),
                  Padding(
                    padding: EdgeInsets.symmetric(horizontal: 20),
                    child: Wrap(
                      spacing: 6, runSpacing: 6, alignment: WrapAlignment.center,
                      children: (auth.skills ?? '').split(',').map((s) => s.trim()).where((s) => s.isNotEmpty).map((skill) =>
                        Container(
                          ...
                          child: Text(skill, style: TextStyle(color: Colors.white, fontSize: 12)),
                        ),
                      ).toList(),
                    ),
                  ),
          ...
          Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                _infoCard(Icons.code_outlined,   "Skills",    (auth.skills ?? '').isEmpty ? 'Not set' : auth.skills!),
